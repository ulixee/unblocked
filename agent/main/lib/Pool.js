...
var _Pool_agentsByBrowserId, _Pool_browserIdByAgentId, _Pool_activeAgentsCount, _Pool_waitingForAvailability;
...
class Pool extends eventUtils_1.TypedEventEmitter {
    ...
    constructor(options = {}) {
        ...
        this.plugins = [];
        _Pool_agentsByBrowserId.set(this, {});
        _Pool_browserIdByAgentId.set(this, {});
    ...
    async getBrowser(engine, hooks, launchArgs) {
        return await this.browserCreationQueue.run(async () => {
            ...
            for (const existingBrowser of this.browsersById.values()) {
                const agents = __classPrivateFieldGet(this, _Pool_agentsByBrowserId, "f")[existingBrowser.id] ?? 0;
                if (agents < this.maxConcurrentAgentsPerBrowser && existingBrowser.isEqualEngine(engine)) {
                    __classPrivateFieldGet(this, _Pool_agentsByBrowserId, "f")[existingBrowser.id] += 1;
                    return existingBrowser;
                }
            }
            __classPrivateFieldGet(this, _Pool_agentsByBrowserId, "f")[browser.id] ??= 0;
            __classPrivateFieldGet(this, _Pool_agentsByBrowserId, "f")[browser.id] += 1;
    ...
    onAgentClosed(closedAgentId) {
        ...
        const browserId = __classPrivateFieldGet(this, _Pool_browserIdByAgentId, "f")[closedAgentId];
        if (__classPrivateFieldGet(this, _Pool_agentsByBrowserId, "f")[browserId]) {
            __classPrivateFieldGet(this, _Pool_agentsByBrowserId, "f")[browserId] -= 1;
            if (__classPrivateFieldGet(this, _Pool_agentsByBrowserId, "f")[browserId] === 0) {
                delete __classPrivateFieldGet(this, _Pool_agentsByBrowserId, "f")[browserId];
            }
            delete __classPrivateFieldGet(this, _Pool_browserIdByAgentId, "f")[closedAgentId];
        }
...
_Pool_agentsByBrowserId = new WeakMap(), _Pool_browserIdByAgentId = new WeakMap(), _Pool_activeAgentsCount = new WeakMap(), _Pool_waitingForAvailability = new WeakMap();
