...
describe('Worker test', () => {
    ...
    beforeAll(async () => {
        ...
        browser = unblocked_agent_testing_1.BrowserUtils.createDefaultBrowser();
    ...
    it('Page.workers', async () => {
        ...
        for (let i = 0; i < 10; i += 1) {
            const isReady = await worker.evaluate('!!self.workerFunction');
            if (isReady)
                break;
    ...
    it('should report errors', async () => {
        const errorPromise = new Promise(resolve => page.on('page-error', resolve));
        await page.evaluate(`(() => {
    ...
    it('should report network activity', async () => {
        ...
        const requestPromise = page.waitOn('resource-will-be-requested', x => x.resource.url.href === url);
    ...
    it('should report network activity on worker creation', async () => {
        ...
        await page.goto(server.emptyPage);
        const url = `${server.baseUrl}/one-style.css`;
        const requestPromise = page.waitOn('resource-will-be-requested', x => x.resource.url.href.includes('one-style.css'));
        await page.evaluate(`(() => {
        new Worker(
          URL.createObjectURL(
            new Blob(
