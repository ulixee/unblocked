        ...
        if (itemToModify.propertyName === '_$value') {
            ...
            if (!OtherInvocationsTracker.basePaths.has(itemToModify.path)) {
                proxyFunction(parent, property, (target, thisArg, argArray) => {
                    const otherInvocation = OtherInvocationsTrackerHere.getOtherInvocation(itemToModify.path, thisArg);
            ...
            OtherInvocationsTracker.addOtherInvocation(itemToModify.path, otherKey, itemToModify.property);
        }
    }
    catch (err) {
