...
test('can resolve a v4 address', async () => {
    await expect((0, lookupPublicIp_1.default)()).resolves.toBeTruthy();
});
test('can resolve an ip address with a mitm socket', async () => {
    ...
    await expect((0, lookupPublicIp_1.default)(lookupPublicIp_1.IpLookupServices.aws, session.requestAgent)).resolves.toBeTruthy();
});
test('should override webrtc ip', async () => {
    const publicIp = await (0, lookupPublicIp_1.default)();
