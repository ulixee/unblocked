...
const testIfNotOnGithubMac = process.env.CI === 'true' && process.platform === 'darwin' ? test.skip : test;
...
<script>
  ...
  window.betrayalResult = {
    ...
    expect(result.stack).toContain(`at iWillBetrayYouWithMyLongName (${koaServer.baseUrl}/${testSpecificPath()}:5:9)`);
});
...
describe('Proxy detections', () => {
 ...
 try {
  ...
  return { path, result };
        ...
        expect(result.path).not.toContain(DomOverridesBuilder_1.injectedSourceUrl);
...
test('should not trigger stack for unhandled error', async () => {
  ...
  <script>
    ...
    await page.goto(`${koaServer.baseUrl}/${testSpecificPath()}`);
    await page.waitForLoad('DomContentLoaded');
    const getterCalled = await page.evaluate('window.getterCalled');
...
test('should allow custom getters on errors (safety check so our logic doesnt break this)', async () => {
  ...
  <script>
    ...
    await page.goto(`${koaServer.baseUrl}/${testSpecificPath()}`);
    await page.waitForLoad('DomContentLoaded');
    const stack = await page.evaluate('window.stack');
