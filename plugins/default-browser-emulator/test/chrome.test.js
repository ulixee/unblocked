...
beforeAll(async () => {
    ...
    ({ chrome, prevProperty } = JSON.parse(Fs.readFileSync(windowChromePath, 'utf8')));
...
test('it should mimic a chrome object', async () => {
    ...
    const script = (0, DomOverridesBuilder_1.getOverrideScript)('window.chrome', {
        polyfill: {
            property: chrome,
            prevProperty,
        },
...
test('it should update loadtimes and csi values', async () => {
    const httpServer = await Helpers.runHttpServer();
    const page = await createPage();
    await page.addNewDocumentScript((0, DomOverridesBuilder_1.getOverrideScript)('window.chrome', {
