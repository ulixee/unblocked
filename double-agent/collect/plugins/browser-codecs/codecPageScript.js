...
exports.default = codecPageScript;
const audioMimeAlternatives = csv((0, fs_1.readFileSync)(`${__dirname}/assets/mime/audio-mimetypes.csv`, 'utf8'))
    .slice(1)
    .map(x => x[1].toLowerCase())
    .filter(Boolean);
const videoMimeAlternatives = csv((0, fs_1.readFileSync)(`${__dirname}/assets/mime/video-mimetypes.csv`, 'utf8'))
    .slice(1)
    .map(x => x[1].toLowerCase())
    .filter(Boolean);
const applicationMimeAlternatives = csv((0, fs_1.readFileSync)(`${__dirname}/assets/mime/application-mimetypes.csv`, 'utf8'))
    .slice(1)
    .map(x => x[1].toLowerCase())
    .filter(Boolean);
const audioMimes = [
...
//# sourceMappingURL=codecPageScript.js.map