...
async function runBrowserstack() {
    ...
    for (const agent of real_user_agents_1.default.all()) {
        ...
        if (browser.name === 'Firefox' && Number(browser.version.major) < 63)
            continue;
        if (browser.name === 'Chrome' && Number(browser.version.major) < 58)
            continue;
        if (browser.name === 'Safari' && Number(browser.version.major) < 11)
            continue;
